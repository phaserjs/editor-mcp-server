...
export const EditableTilemapLayerSchema = z.object({
    ...
    data: z.array(z.array(z.number())).describe("The tile data of the layer. Each item is a tile ID, or -1 for an empty tile. The tile ID is a global ID that is computed by the index of the tile in the tileset. If there are more than one tileset, then the global ID is computed by adding the length of each tileset."),
